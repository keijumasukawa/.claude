...
2. リポジトリ運用規程「リポジトリの設定」の設定を、後述のコマンドで適用する。
3. 空の README.md を `docs/readme` ブランチの単独プルリクエスト(コミットメッセージ及びタイトルは `docs: README を追加`)で登録する。本文はテンプレートの全項目を次の内容で簡素に埋め、詳述しない。
   - 目的: README の登録
   - 変更内容: README の登録
